...
class Attention(nn.Module):
    def __init__(self, state_dim, hypernet_embed, embed_dim): # embed_dim=mixdim
    ...
    def forward(self, x, state):
        x = x.transpose(1, 2)  # 张量(tensor)的转置操作,交换1 2 两个维度
        b, t, _ = x.shape  # 用于获取张量x的形状信息。
...
class Attentionmix(nn.Module):
    def __init__(self, args):
        ...
        if args.two_hyper_layers == False:
        ...
        elif args.two_hyper_layers == True:
        ...
        self.attn = Attention(self.state_dim, hypernet_embed, self.embed_dim)
